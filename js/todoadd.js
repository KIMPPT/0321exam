...
function addtext(e)
{
    ...
    checkbox.type="checkbox"; //input태그의 타입을 checkbox로 만듬
...
function deleteli(e){
    ...
    if(box.checked) finish--; 
    //li 삭제 때 완료한 할 일이면 finish 카운트도 내려가게 설정
    //이 설정을 하지 않으면 체크 된 것을 삭제 할 때 완료 카운트가 내려가지 않아서 값이 일치하지 않는다
...
function changecolor(e){
    ...
    if(checkbox.checked) //checkbox의 체크 유무에 따라 투두 입력으로 온 문장 색 조절+완료한 일 카운트 증감 조절
    {
        text.style.color="white";
        finish++; //체크하기 때문에 완료한 일 카운트 증가
    }
    else 
    {
        text.style.color="";
        finish--; //체크를 해제하기 때문에 완료한 일 카운트 감소
    }
    h3.innerHTML=`전체 할 일 ${all} / 완료한 할 일 ${finish}`;
}
